...
public sealed class PurgeSweepStrategyEndToEndTests(PostgresFixture fixture)
{
    ...
    public async Task EraseAsync_Uses_An_Or_Subject_Predicate_When_Multiple_Subject_Matches_Are_Provided()
    {
        ...
        var affected = await strategy.EraseAsync(
            ...
            new ErasureSubjectPredicate(
                [
                    new ErasureSubjectMatch(nameof(PurgeCandidateRecord.Id), "SubjectId", firstSubjectId),
                    new ErasureSubjectMatch(nameof(PurgeCandidateRecord.Id), "DelegateSubjectId", secondSubjectId),
                ]
            ),
            new TenantContext(tenantId, "uk", new Dictionary<string, string>()),
            now,
